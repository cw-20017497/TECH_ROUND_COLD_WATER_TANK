...
U8 pkt_recv[ MAX_COMM_RX_BUF_SZ ];
I16 pkt_recv_len = 0;

/* Send buffer, length */
//volatile U8 pkt_send[ MAX_COMM_TX_BUF_SZ ];
U8 pkt_send[ MAX_COMM_TX_BUF_SZ ];
I16 pkt_send_len = 0;
...
static parser_rx_list_t parser_rx_list[] = 
{
...
static parser_tx_list_t parser_tx_list[] = 
{
...
void SendPacketHandler( void )
{
    ...
    for( i = 0 ; i < MAX_PARSER_TX_NUM ; i++ )
    {
        ...
        if( IsExpiredTimer( p_list->TimerId ) == TIMER_EXPIRE )
        {
            ...
            pkt_send_len = p_list->make_pkt( (CommHeader_T *)p_comm, &pkt_send[0] );
            if( pkt_send_len > 0 )
            {
                /* MAKE CRC-16 */
