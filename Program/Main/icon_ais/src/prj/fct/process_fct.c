...
static void InitLoad(void)
{
    ...
    //R_UART0_Stop(); // COMP
    //R_UART1_Stop(); // WIFI
    R_Config_UART3_Stop(); // EOL


    R_Config_TAU0_2_Stop(); // HEATER CONTORL TIMER ISR
...
static void CheckCommEol(void)
{
    ...
    switch( mu8Step )
    {
        ...
        case 1:
            PIN_WRITE(P_EOL_TX) = 1;
            if( PIN_READ(P_EOL_RX) == 1 )
            {
                mu8Step++;
            }
            break;
        ...
        case 2:
            PIN_WRITE(P_EOL_TX) = 0;
            if( PIN_READ(P_EOL_RX) == 0 )
            {
                mu8Step++;
            }
            break;
...
void ProcessFctLoadTest(void)
{
...

    {
        ...
        if( IsSetFctTestInputVal( MK_FCT_KEY_SAVE ) == TRUE )
        {
            ...
            {
                mu8OneShot = TRUE;
                StartTimer( TIMER_ID_COMP, TIMER_SEC(0) );
            }
