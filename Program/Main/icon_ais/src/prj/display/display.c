...
static void DispSegment(U8 mu8Seg, U8 mu8Val)
{
    ...
    {
        mLedId = (LedId_T)mu8Seg;
        HAL_TurnOnOffLED( mLedId,      ((mu8Val & 0x04) >> 2) );
        HAL_TurnOnOffLED( (LedId_T)(mLedId + 1),  ((mu8Val & 0x02) >> 1) );
    }
    else
    {
        mLedId = (LedId_T)mu8Seg;
        HAL_TurnOnOffLED( mLedId,     ((mu8Val & 0x40) >> 6) );
        HAL_TurnOnOffLED( (LedId_T)(mLedId + 1), ((mu8Val & 0x20) >> 5) );
        HAL_TurnOnOffLED( (LedId_T)(mLedId + 2), ((mu8Val & 0x10) >> 4) );
        HAL_TurnOnOffLED( (LedId_T)(mLedId + 3), ((mu8Val & 0x08) >> 3) );
        HAL_TurnOnOffLED( (LedId_T)(mLedId + 4), ((mu8Val & 0x04) >> 2) );
        HAL_TurnOnOffLED( (LedId_T)(mLedId + 5), ((mu8Val & 0x02) >> 1) );
        HAL_TurnOnOffLED( (LedId_T)(mLedId + 6), (mu8Val & 0x01)  );
    }
}
...
void DispCompRps(void)
{
    TurnOffAllIcon();

    if( dbg_disp_comp_rps == 1 )
    {
        DispWaterOutAmount( GetCompBldcTargetRps() );

        HAL_TurnOnOffLED( ICON_AM, ON );
        HAL_TurnOnOffLED( ICON_PM, OFF );
    }
    else if( dbg_disp_comp_rps == 2 )
    {
        DispWaterOutAmount( GetCompBldcCurrentRps() );

        HAL_TurnOnOffLED( ICON_AM, OFF );
        HAL_TurnOnOffLED( ICON_PM, ON );
    }
    else if( dbg_disp_comp_rps == 3 )
    {
        U16 temp;

        temp = (U16)( GetTemp( TEMP_ID_COLD_WATER) * 10.0f );
        DispWaterOutAmount( temp );
    }
}
