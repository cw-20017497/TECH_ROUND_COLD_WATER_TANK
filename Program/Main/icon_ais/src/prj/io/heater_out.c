...
#pragma interrupt HeaterOutInterrupt(vect=INTTM02)
...
static void SetTriacOutput(U16 mu16Output)
{
    ...
    // 히터의 총 watt
    TriacWatt.Heater = (U32)GetHeaterBaseWatt();
...
void SetHeaterOutput(U16 mu16Output)
{
    ...
    else if( HeaterOut.Region == REGION_DOUBLE )
    {
        ...
        {
            U16 mu16TriacOutput = 0;

            mu16TriacOutput = HeaterOut.Output - RELAY_HEATER_OUT;
            if( mu16TriacOutput >= TRIAC_HEATER_OUT )
            {
                SetTriacOutput( TRIAC_HEATER_OUT );
            }
            else
            {
                SetTriacOutput( mu16TriacOutput );
            }
        }
    }
    else
...
static void __near HeaterOutInterrupt(void)
{
    EI();
    ProcessHeaterOut();
}
