...
// I8 ai8WifiRxBuff[LENGTH_WIFI_ALL_MAX]; // Ã³¸®µÇ±â Àü¿¡ µ¥ÀÌÅÍ°¡ µé¾î¿À´Â°æ¿ì ¹öÆÛ
...
static void RunRxData ( const WifiRxAtList_T *pList, U8 mSoundSet );
...
{
    const WifiRxAtList_T *pList = NULL;
    U8 i = 0U;
    U8 mSound = TRUE;
...
static void RunRxData ( const WifiRxAtList_T *pList, U8 mSoundSet )
{
    ...
    if ( pList->Disp != WIFI_NULL )
    {
        if ( ( pList->Disp >= WIFI_DISP_AP1 ) && ( pList->Disp <= WIFI_DISP_AP5 ) )
        {
            ...
            {
                WifiRx.u8DisplayConnectCount = SET; // ¿¬°á²÷±è ÈÄ 1ºÐ Ä«¿îÆ® ½ÃÀÛ
            }
        }
        else
        {
...
static U8 RxCnt(void)
{
    ...
    {
        SetWifiSendStatus( TX_ATCMD, WIFI_AT_BLE_OFF ); // BLE Æä¾î¸µ¸ðµå OFF
        SetWifiIndicateStatus( STATUS_PARING, WIFI_AP_STEP_CLEAR );
    }
    WifiRx.u8DisplayConnectCount = CLEAR; // ¼­¹ö¿¬°á²÷±è 1ºÐ Ä«¿îÆ® Å¬¸®¾î (¼­¹ö¿¬°á¿Ï·á)
    ...
    if ( GetWifiApStatus( STATUS_AP_FIRST ) == CLEAR ) // $)CCVCJ Fd>n85 @z@e
    {
        ...
        SetWifiIndicateStatus( STATUS_PARING, WIFI_AP_STEP_CLEAR );  // ÃÖÃÊ Æä¾î¸µ ±¹°¡ÄÚµå Àû¿ë ÈÄ ¸ðµâ ResetÀ¸·Î BLE OFF°¡ ¾È¿À´Â °æ¿ì¿¡¼­ ÀÚµ¿À¸·Î BLE OFF
