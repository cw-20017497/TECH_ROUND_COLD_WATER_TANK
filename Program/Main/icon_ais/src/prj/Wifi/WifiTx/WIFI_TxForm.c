...
void* GetWifiTxTypeEventList ( U8 mType )
{
    if ( mType == TX_ATCMD )
    {
        return (void *)WifiSendAtList;
    }
    else /* ( mType == TX_DATA ) */
    {
        return (void *)WifiSendDataList;
    }
}
...
void* GetWifiTxCmdEventList ( U8 mType )
{
    if (( mType == BLEDATA ) )
    {
        return (void *)Wifi_BLE_TxCmdList;
    }
    else 
    {
        return (void *)WifiTxCmdList;
    }
}
...
static void ExceptAtCmd ( U8 mExcept, I8* pBuf, U16 mLen )
{
   ...
   static I8 mai8ModelName[LENGTH_MODEL_NAME+1];
    ...
    {
        far_sprintf_s( mai8ModelType, 0, (I8 *__near)FOTA_TYPE_CHP );
        far_sprintf_s( mai8ModelName, 0, (I8 *__near)MODEL_NAME_CHP );
    }
    else if ( GetUserInterface(USER_MODEL_TYPE) == USER_MODEL_CP )
    {
        far_sprintf_s( mai8ModelType, 0, (I8 *__near)FOTA_TYPE_CP );
        far_sprintf_s( mai8ModelName, 0, (I8 *__near)MODEL_NAME_CP );
    }
    else if ( GetUserInterface(USER_MODEL_TYPE) == USER_MODEL_HP )
    {
        far_sprintf_s( mai8ModelType, 0, (I8 *__near)FOTA_TYPE_HP );
        far_sprintf_s( mai8ModelName, 0, (I8 *__near)MODEL_NAME_HP );
    }
    else
    {
        far_sprintf_s( mai8ModelType, 0, (I8 *__near)FOTA_TYPE_CHP );
    ...
    {
        far_sprintf_s( &pBuf[mLen], 0, (I8 *__near)WIFI_VERSION );
    }
    else if ( mExcept == TO_APMODE )
    {
        mLen += far_sprintf_s( &pBuf[mLen], 0, (I8 *__near)COWAY_SSID );
        far_sprintf_s( &pBuf[mLen], 0, mai8ModelName );
    }
    else if ( mExcept == TO_SCONN )
    {
        mLen += far_sprintf_s( &pBuf[mLen], 0, (I8 *__near)TEST_MODEM_SSID );
        far_sprintf_s( &pBuf[mLen], 0, (I8 *__near)TEST_MODEM_PW );
    }
